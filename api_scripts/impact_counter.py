...
for year in years:
    data = client.get_awards_from_event(year)

    for award in data:
        if award['award_type'] == AWARD_TYPES["CHAIRMANS"]:
            team_data = client.get_team_info(award['recipient_list'][0]['team_key'])
            print(award['recipient_list'][0]['team_key'], award["name"], year, team_data["state_prov"])
